...
const USER_COOLDOWN_MS = 8 * 1000   // one API reply per user per 20s
const DAILY_CAP   = 600      // total API replies per day, then it goes quiet
...
// ─── anthropic ───────────────────────────────────────────────────────────
async function ask(question, askerName) {
  ...
  // log cache performance so you can see the savings working
  const u = data.usage || {}
  console.log(
...
bot.on('message', async msg => {
  ...
  // only respond when spoken to: tagged, or replying to the bot
  const tagged = BOT_USERNAME && lower.includes('@' + BOT_USERNAME)
  const repliedToBot = msg.reply_to_message?.from?.username?.toLowerCase() === BOT_USERNAME
  if (!tagged && !repliedToBot) return
  ...
  try {
    dailyCount++
    const answer = await ask(question, name)
    if (answer) {
      await bot.sendMessage(chatId, answer, { reply_to_message_id: msg.message_id })
    }
    console.log(`✅ replied to ${name} (${dailyCount}/${DAILY_CAP} today)`)
  } catch (e) {
...
// follow the group if it upgrades to a supergroup
